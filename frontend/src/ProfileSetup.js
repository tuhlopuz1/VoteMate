...
const ProfileSetupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const descriptionResponse = await fetch('https://api.vote.vickz.ru/api/v2/update-profile', {
        method: 'PUT',
        headers: {
      ...
      localStorage.setItem('description', formData.bio)
      // Второй запрос: загружаем аватар
      if (formData.avatar) {
        const avatarData = new FormData();
        avatarData.append('file', formData.avatar);

        const avatarResponse = await fetch('https://api.vote.vickz.ru/api/v2/profile-picture', {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`
          },
